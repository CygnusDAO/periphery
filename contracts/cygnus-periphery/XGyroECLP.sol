...
contract XGyroECLP is CygnusAltairX, ICygnusAltairCall {
    ...
    function _tokenWeights(IGyroECLPPool lpTokenPair) private view returns (uint256 weight0, uint256 weight1) {
        ...
        // Calculate virtual base value - For one of the tokens this has no effect
    ...
    function _convertUsdToLiquidity(
        ...
        // Mint requested BPT. Substract 1 from the BPT and if calculations are correct we will be left over
        // with 0 of token0 or token1 and some small amount of the other token
        VAULT.joinPool(
            poolId,
            address(this),
            address(this),
